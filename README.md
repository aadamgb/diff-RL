...
A PyTorch-based differentiable framework for training neural network policies to control a 2D bicopter. The project supports three control modes [(SRT, CTBR, LV)](#action-output) and includes real-time visualization in Pygame. Training can be executed in parallel environments at the GPU for efficient data colletion.
...
The bicopter's objective is to match the state of a reference target defined by position $p_{ref}$, velocity $v_{ref}$, and acceleration $a_{ref}$. The target's trajectory is generated by summing random sinusoidal harmonics, providing diverse training signals. The policy is trained by minimizing a weighted loss function:
