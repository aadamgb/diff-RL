...
The bicopter's objective is to track a reference trajectory defined by position $\mathbf{p}_{ref}$, velocity $\mathbf{v}_{ref}$, and acceleration $\mathbf{a}_{ref}$. The reference trajectory is generated by summing random sinusoidal harmonics, providing diverse training signals. The policy is trained by minimizing a weighted loss function:
